...
function App() {
  ...
  function validateWord(word) {
    ...
    if (
      !acceptableWords.includes(word.toLowerCase()) &&
...
function Header() {
  return (
    <div className="w-full border-b border-slate-500 p-2 mb-6 pb-6 text-center">
      <h1 className="text-3xl font-bold text-white">WORDLE</h1>
      <div className="text-red-500 font-serif bg-transparent p-0.5 border-4 border-red-500 rounded text-xs font-bold italic transform -rotate-12 w-24 absolute top-8 left-1/2">
        After Dark
      </div>
    </div>
  );
            ...
            <div className="relative p-6 flex-auto">
              <p className="my-4 text-blueGray-500 text-lg leading-relaxed">
                {showModal.status === "win"
                  ? `You Won! The word was ${answerWord.toUpperCase()}`
                  : `You lost. The word was ${answerWord.toUpperCase()}`}
              </p>
            </div>
            {/*footer*/}
...
function GameTile({ value, status }) {
  let bgColor = "";
  if (status === "correct") {
    bgColor = "bg-green-400 border-0";
  } else if (status === "close") {
    bgColor = "bg-yellow-400 border-0";
  } else if (status === "incorrect") {
    bgColor = "bg-slate-500 border-0";
  }

  return (
    <p
      className={`text-white border h-12 w-12 md:h-16 md:w-16 border-slate-300 m-1 text-4xl font-bold flex items-center justify-center ${bgColor}`}
    >
      {value}
    </p>
  );
